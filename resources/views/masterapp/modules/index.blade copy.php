...
<script>
    ...
    $(document).ready(function() {
      ...
      $(document).ajaxError(function(event, xhr, settings, thrownError) {
            ...
            const moduleDataRoute = "{{ route('masterapp.modules.data') }}";
            ...
            if (settings.url.indexOf(moduleDataRoute) !== -1) {
                ...
                console.log('Caught error for modules data:', xhr); // For debugging
        ...
        CRUDManager.init({
            resource: 'Module',
            serverSide: {
                url: "{{ route('masterapp.modules.data') }}",
                columns: [{
                        data: 'checkbox',
                        name: 'checkbox',
                        orderable: false,
            ...
            endpoints: {
                create: "{{ route('masterapp.modules.create') }}",
                edit: "/modules/{id}/edit",
                delete: "/modules/bulk-delete"
            },
            buttons: [
                 @can('create-modules')
                {
